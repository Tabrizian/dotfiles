# Exports {{{
export GITHUB_USER="tabrizian"
export PATH=~/.local/bin:/usr/local/go/bin:/opt/rubies/ruby-2.3.0/bin:/usr/local/bin:/usr/bin:/bin:/usr/sbin:/sbin:/usr/local/bin/ghdl/bin:.:/opt/electron-linux # Reorder PATH so local bin is first
...
PROMPT='
${PR_GREEN}iman@$(hostname).%{$reset_color%} ${PR_BOLD_YELLOW}$(current_pwd)%{$reset_color%} $(git_prompt_string)
$(prompt_char) '
...
# gitignore {{{
function ignore {
until $(curl --output /dev/null --silent --head --fail http://gitignore.io/api/$1); do
  printf '.'
  sleep 5
done
}
